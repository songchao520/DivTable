  ...
  CreateZcTable.prototype = {
  	initTable:function(){
  		...
  		var isauto = this.options.isauto;
			var islinkbootstrap = this.options.islinkbootstrap;
  		...
  		var sumwidth = 0
  		if(islinkbootstrap == "false"){
  			sumwidth = sumwidth+parseFloat(colsnumber*parseFloat(contentwidth))+parseFloat(colsnumber)+1;
  			if(isauto=="false"){
  				if(isShowIndex == "true"){
  					sumwidth = sumwidth+parseFloat(indexwidth)+1;
  				}
  				if(isShowIcon == "true"){
  					sumwidth = sumwidth+parseFloat(iconwidth)+1;
  				}
  				if(isShowCheckbox == "true"){
  					sumwidth = sumwidth+parseFloat(checkboxwidth)+1;
  				}
  			}
  		}else{
  		...
  		if(islinkbootstrap == "false"){
  			var thiswidth = parentdiv - colsnumber -1;
  			if(isauto == "true"){
  				if(isShowIndex == "true"){
  					thiswidth = thiswidth - indexwidth -1;
  				}
  				if(isShowIcon == "true"){
  					thiswidth = thiswidth - iconwidth -1;
  				}
  				if(isShowCheckbox == "true"){
  					thiswidth = thiswidth - checkboxwidth -1;
  				}
  		...
  		for(var i=0;i<colclass.length;i++){
	  		...
	  		elements.children(".tableth").children(".zctablethtd").each(function(){
	  			...
	  			if($(this).hasClass(colclass[i]+"th")|| $titlename==colclass[i] ){
	  				...
	  				if(options.islinkbootstrap != "false"){
	  					//如果是自适应模式将重新赋值；
	  				...
	  				if(options.islinkbootstrap == "false"){
	  					if(options.isauto == "true"){
		  					var trsum = $(this).parent().children(".zctablethtd").length;
								var hidesum = $(this).parent().children(".create_table_hide").length;
								nowwidths = (trsum-hidesum+1)*thiswidth/(trsum-hidesum);
								//alert((trsum-hidesum)*thiswidth);
								$(this).parent().children(".zctablethtd").css("width",nowwidths+"px");
								$(this).parent().parent().children(".zctabletr").children(".zctabletd").css("width",nowwidths+"px");
								$(this).parent().parent().children(".sumtr").children(".sumtd").css("width",nowwidths+"px");
								...
								$(this).parent().children("div").each(function(){
									if(!$(this).hasClass("create_table_hide")){
										newsumwidth = parseFloat(parseFloat(newsumwidth)+1+parseFloat($(this).width()));
									}
									
								});
								newsumwidth = newsumwidth+2;
								$(this).parent().css("width",newsumwidth+"px");
								$(this).parent().parent().css("width",newsumwidth+"px");
		  				}
	  				}
