...
;(function($,window,document,undefined){
  ...
  CreateZcTable.prototype = {
  	...
  	setEditHtmlCustom:function(name,inputhtml){
  		edithtmlparameters[name] = inputhtml;
  	},
  	closeEditCustom:function(showtexts){
  		
  		var elements = this.$element;
  		elements.find(".create_table_Edit_input").each(function(){
  			var value = $(this).val();
			...
			$parent.html(value);
			if(functions["celleditsuccess"]!=null && functions["celleditsuccess"]!=""){
				$(functions["celleditsuccess"]);
			}
  		})
  ...
  //根据ID向某个单元格塞入特定html
	setHtmlColByID:function(jsonobj){
  	...
  	AddCellEditSuccess:function(callback){
  		functions["celleditsuccess"] = callback;
  		
  	},
  	AddCellClick:function(callback){
  		functions["cellclick"] = callback;
  	},
  	AddCheckBox:function(callback){
  		functions["checkboxclick"] = callback;
  	},
  	AddMonitor:function(){
  		if(inittableindex==1){
			...
			$(document).on("click",".page_number_btn",function(){
				...
				if($(this).parent().parent().parent().attr("pageTable")!=null && $(this).parent().parent().parent().attr("pageTable")!=""){
					
					var biaos = $(this).parent().parent().parent().attr("pageTable");
					elements = $(biaos);
				}
				...
				var pagenum = Math.ceil(maxindex/pagesize);
		  		...
		  		if($(this).parent().parent().parent().attr("pageTable")!=null && $(this).parent().parent().parent().attr("pageParent")!=""){
		  			
					var pageParent = $(this).parent().parent().parent().attr("pageParent");
					$(pageParent).find(".zctable_page_ul").html(pagehtml);
				}else{
					elements.find(".zctable_page_ul").html(pagehtml);
				}
			...
			$(document).on("click",".checkboxtrinput",function(e){
				e.stopPropagation();
				if(functions["checkboxclick"]!=null && functions["checkboxclick"]!=""){
					$(functions["checkboxclick"]);
				}
			})
			//监听表头行选择
			...
			$(document).on("click",".zctable_title .zctablethtd",function(){
			...
			$(document).on("click",".zctabletd",function(){
				...
				if(functions["cellclick"]!=null && functions["cellclick"]!=""){
					$(functions["cellclick"]);
				}
			...
			$(document).on("click",".create_table_Edit",function(e){
				...
				if($(this).hasClass("create_table_Edit_Custom")){
					...
					if(edithtmlparameters[names] != null && edithtmlparameters[names] !=""){
						inputhtml = edithtmlparameters[names];
					}else{
						alert("请先自定义html")
					}
				...
				$(this).html(inputhtml);
				if($(this).hasClass("create_table_Edit_Custom")){
					$(this).find("input").addClass("create_table_Edit_input")
				}
				var $children = $(this).children(".create_table_Edit_input");
				if($(this).hasClass("create_table_Edit_Custom")){
					$children = $(this).find(".create_table_Edit_input");
				}
